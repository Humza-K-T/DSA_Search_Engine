...
class InvertedIndex:
        ...
        with open(ForwardIndexPath, 'rb') as ForwardIndexData1:
            ...
            for i, InvIndBar in enumerate(InvInd1):
                if not len(InvIndBar) == 0:
                    filename = os.path.join(self.temp_path, f"{i:03}_inverted_{ntpath.basename(ForwardIndexPath)}")
    ...
    def MergeIndex(self):
        ...
        for i in range(self.lexiconsize // self.barrelsize + 1):
            concerned_indexes = [temp_index for temp_index in InvIndTemp if temp_index.startswith(f"{i:03}_inverted_")]
            ...
            for concerned_index in concerned_indexes:
                with open(os.path.join(self.temp_path, concerned_index), 'rb') as TemporaryIndexData:
